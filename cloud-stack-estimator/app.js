...
function recommendationText(results) {
  ...
  const bestBlend = results.reduce((acc, item) => {
...
function render(results, usdInr, fxLabel) {
...
async function resolveFxRate(useLive) {
  if (!useLive) {
    return { rate: DEFAULT_USD_INR, label: `Using fallback FX rate: 1 USD = ${DEFAULT_USD_INR} INR.` };
  }
...
async function recalculate() {
  ...
  const fx = await resolveFxRate(useLiveFx);

  const results = Object.values(pricing).map((provider) => {
  ...
  render(results, fx.rate, fx.label);
}
